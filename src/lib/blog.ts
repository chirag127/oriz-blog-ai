...
export function verticalFor(category: string): Vertical {
  ...
  if (/(eval|benchmark|metric|test|guardrail|safety|red.?team|hallucin)/.test(c)) return 'health'
  if (/(retriev|rag|embed|vector|index|search|context|memory)/.test(c)) return 'culture'
  if (/(model|llm|train|fine.?tun|weight|transformer|architecture|inference|prompt)/.test(c))
    return 'finance'
  if (/(system|infra|pipeline|deploy|serving|agent|tool|mcp|ops|engineer|code|scal)/.test(c))
    return 'dev'
  return 'personal'
}
